...
public interface ClassNoticeRepository extends JpaRepository<ClassNotice,Long> {
    ...
    @Query("SELECT DISTINCT not FROM ClassNotice not JOIN not.classNoticeType cnt WHERE not.classroom.activated = true AND not.teacher.activated = true AND not.classroom.id = :classroom_id AND cnt IN :types")
    ...
    @Query("SELECT DISTINCT not FROM ClassNotice not JOIN not.classNoticeType cnt WHERE not.classroom.activated = true AND not.teacher.activated = true AND not.classroom.id = :classroom_id AND cnt In :types AND not.title LIKE CONCAT('%', :notice_title, '%') ")
